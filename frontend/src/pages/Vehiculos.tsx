...
export default function Vehiculos() {
  ...
  const vehiculosFiltrados = vehiculos.filter((vehiculo) => {
    ...
    if (tipoFiltro === 'placa') {
      return vehiculo.placa.toLowerCase().includes(valorBusqueda);
    } else if (tipoFiltro === 'modelo') {
      return vehiculo.modelo.toLowerCase().includes(valorBusqueda);
    } else if (tipoFiltro === 'marca') {
      return vehiculo.marca.toLowerCase().includes(valorBusqueda);
    }
    return true;
  });
  ...
  const vehiculosPaginados = vehiculosFiltrados.slice(startIndex, endIndex);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await api.vehiculos.eliminar(vehiculoToDelete.id);
      setVehiculos(vehiculos.filter((v) => v.id !== vehiculoToDelete.id));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.placa.trim() || !formData.modelo.trim() || !formData.marca.trim()) {
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="bg-primary text-primary-foreground rounded-t-lg px-4 py-3 flex items-center justify-between">
          <h1 className="text-xl font-semibold">Vehículos de Transporte</h1>
          <Button
            onClick={handleNew}
        ...
        <div className="bg-muted/50 px-4 py-4 border-x border-border space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Select value={tipoFiltro} onValueChange={(value: 'placa' | 'modelo' | 'marca') => setTipoFiltro(value)}>
              ...
              {vehiculosPaginados.length === 0 ? (
                ...
                vehiculosPaginados.map((vehiculo, index) => (
                  <tr key={vehiculo.id} className="hover:bg-muted/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-sm">{vehiculo.placa}</td>
                    <td className="px-4 py-3 text-sm">{vehiculo.modelo}</td>
                    <td className="px-4 py-3 text-sm">{vehiculo.marca}</td>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <strong>{vehiculoToDelete?.placa}</strong>? Esta acción no se puede deshacer.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
